# 환경 구축 기록 (engineering notes)

> **읽기 전에**
>
> 이 문서는 정리된 설명서가 아니라 **환경을 구축하면서 그때그때 남긴 작업 기록**입니다.
> 막힌 지점과 원인, 해결 방법이 시간 순으로 쌓여 있습니다. 같은 함정을 다시 밟지
> 않으려고 남긴 것이라 PR 에 함께 넣었습니다.
>
> 두 가지만 감안하고 봐주십시오.
>
> 1. **폴더 구조가 다릅니다.** 개발 환경은 모든 파일이 한 폴더에 있는 평면 구조이고,
>    이 PR 은 `app/` `tests/` `scripts/` 로 나눈 구조입니다.
> 2. **여기 언급된 파일이 PR 에 다 있지는 않습니다.** 진단용 보조 스크립트와 1회성
>    환경 구축 스크립트는 팀원들이 실행할 수 없어 일부러 제외했습니다.
>    PR 에 실제로 포함된 파일은 `../README.md` 의 구조 표를 보시면 됩니다.
> 3. **옛 파일명이 나옵니다.** `drone_link.py` 는 현재 `app/mavlink_drone.py` 입니다.
>    기록의 원문을 살리려고 고치지 않았습니다.
...
mavlink_drone.py가 미들웨어(박병언)와 맞닿는 프로토콜 레이어.

[핵심 파일]
- drone_interface.py  프로토콜 공통 계약. 아래 두 어댑터가 이걸 구현한다
- mavlink_drone.py    MAVLink 어댑터 (연결/ARM/이륙/이동/착륙/텔레메트리)
                       예전 이름은 drone_link.py 였다. 이 문서 뒷부분의
                       구버전 기록에도 옛 이름이 남아 있다
- ros2_drone.py       AP_DDS(ROS 2) 어댑터. 같은 계약의 다른 구현
